...
function iniciarCronometro() {
  ...
  intervaloCronometro = setInterval(function () {
    ...
    cronometroElement.innerHTML = `${minutos.toString().padStart(2, '0')}:${segundos.toString().padStart(2, '0')}`;
...
function startStopCronometro() {
  if (!cronometroRodando) {
    iniciarCronometro();
    document.getElementById("startStopBtn").innerHTML = "Pausar";
  }
...
const questions = [
  ...
  {question: '3- Se você me multiplica por 7, subtrai 5 e depois divide por 3, o resultado é 4. Qual é o meu número?',answer:'11'}
];
...
function checkAnswer() {
  ...
  if (userAnswer === currentQuestion.answer.toLowerCase()) {
    ...
    if (currentQuestionIndex < questions.length) {
      ...
      }, 1000); //tempo que a msg fica na tela
    }
  } else {
